# Leer datos
adult.training.full <- read.csv("adult-dataset/adult.training.csv", header = TRUE, sep = ",")
adult.test <- read.csv("adult-dataset/adult.test.csv", header = TRUE, sep = ",")
...
# -- create subgrupos para entrenamiento
library(caret)
folds <- createFolds(training_withoutNoise$Class, 5)
training_fold <- training_withoutNoise[folds[[1]],]

# Árbol de decisión con rpart (usaremos training_withoutNoise, en lugar de folds)
...
#Caret tiene más de 100 modelos de predicción con ejemplos útiles. 
